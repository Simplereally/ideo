...
export function PromptComposer() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (capturedState.provider === "google") {
        ...
        if (!response.ok) {
          throw new Error(
            data.error?.message || "Failed to generate via Vertex",
          );
        }
  ...
  const providerLabel =
    PROVIDER_SHORT_LABELS[state.provider] ?? state.provider;
      ...
      <div className="w-full max-w-2xl flex flex-col gap-3 pointer-events-auto">
        <div
          className={cn(
            ...
            "shadow-[0_8px_40px_-12px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.03)]",
            "border border-black/[0.04]",
            isGenerating &&
              "opacity-50 scale-[0.98] blur-sm pointer-events-none",
            "minimal-focus",
          )}
        >
          {/* Main Input Area */}
          <div className="px-5 pt-5 pb-2">
            <textarea
              ...
              className={cn(
                ...
                "font-serif text-xl md:text-2xl leading-snug placeholder:text-neutral-300",
                "selection:bg-amber-500/15 selection:text-amber-900",
              )}
          ...
          <div className="flex items-center justify-between gap-3 px-3 pb-3 pt-1">
            ...
            <div className="flex items-center gap-1 ml-1.5">
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[12px] font-medium text-neutral-500 hover:bg-black/[0.04] hover:text-neutral-800 transition-colors"
              >
                <span
                  className={cn(
                ...
                />
                <span className="text-neutral-600">{providerLabel}</span>
                <span className="text-neutral-300">/</span>
                <span className="text-neutral-900 font-semibold">{modelLabel}</span>
              </button>
              ...
              >
                ...
                <span className="text-neutral-900 font-semibold">{state.aspectRatio}</span>
              </button>
            </div>
                ...
                {canGenerate && !isGenerating && (
                  ...
                  >
                    <Button
                      ...
                      className="rounded-xl bg-neutral-900 text-white hover:bg-neutral-800 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 px-4 h-8 shadow-sm font-sans font-medium text-[12px] tracking-tight"
                    >
                ...
                {!hasActiveKey && !isGenerating && (
                  ...
                  >
                    <Button
                      ...
                      className="rounded-xl border-black/[0.08] text-neutral-700 hover:bg-black/[0.03] h-8 font-sans font-medium text-[12px] tracking-tight"
                    >
                      Connect API Key
                    </Button>
                  </motion.div>
        ...
        <AnimatePresence>
          {!hasImage && (
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="text-center text-[12px] text-neutral-400 font-sans tracking-tight pt-1"
            >
              Press{" "}
              <kbd className="px-1.5 py-0.5 bg-black/[0.04] border border-black/[0.06] rounded-md text-[10px] mx-0.5 font-sans text-neutral-500">
                Enter
              </kbd>{" "}
              to generate,{" "}
              <kbd className="px-1.5 py-0.5 bg-black/[0.04] border border-black/[0.06] rounded-md text-[10px] mx-0.5 font-sans text-neutral-500">
